...
utils.$document.ready(function () {
  ...
  var $logoC = $('#logoCor');
  var $navItems = $('.nav-link.dropdown-toggle');
  ...
  if ($navbar.length) {
    ...
    utils.$window.scroll(function () {
      ...
      alpha >= 1 && (alpha = 1);
      $navbar.css({
        'background-color': "rgba(" + 255*alpha + ", " + 255*alpha + ", " + 255*alpha + ", " + alpha + ")"
      });
      $logoC.css({
        'opacity': alpha
      });
    ...
    utils.$window.resize(function () {
      if (utils.$window.width() > utils.breakpoints[breakpoint]) {
        return $navbar.removeClass('bg-light');
      }

      if (!$navbar.find('.navbar-toggler').hasClass('collapsed')) {
        return $navbar.addClass('bg-light');
      }
    ...
    $navbar.on('show.bs.collapse hide.bs.collapse', function (e) {
      $(e.currentTarget).toggleClass('bg-light');
    });
  }
});
/*-----------------------------------------------
...
utils.$document.ready(function () {
  ...
  }); // Focus input field and scroll to bottom when form is shown

  $(Selectors.KANBAN_COLLAPSE).on(Events.SHOWN_BS_COLLAPSE, function (e) {
    var $this = $(e.currentTarget);
    var $container = $this.closest(Selectors.KANBAN_ITEMS_CONTAINER);
    var containerHeight = $container[0].scrollHeight;
    $container.find(Selectors.ADD_CARD).focus();
    $container.scrollTop(containerHeight);
  }); // Track form added and toggle Add card button when form is toggled

  $(Selectors.KANBAN_COLLAPSE).on(Events.TOGGLE_BS_COLLAPSE, function (e) {
    var $this = $(e.currentTarget);
    var $column = $this.closest(Selectors.KANBAN_COLUMN);
    $column.toggleClass(ClassNames.FORM_ADDED);
    $column.find(Selectors.BTN_ADD_CARD).parent().toggle();
...
// utils.$document.ready(function () {
//   $('a[href="#admserver"]').on('click', function(event) {
//     event.preventDefault();
//     var headerHeight = $('.fixed-top').outerHeight(); // Substitua com a classe ou ID do seu cabeçalho
//     var scrollToPosition = $('#admserver').offset().top - (headerHeight + 100);
//     $('html, body').animate({
...
utils.$document.ready(function () {
  ...
  var waypoint = new Waypoint({
    ...
    handler: function(direction) {
      initializeCounterUp();
      this.destroy(); // Remove o waypoint após a animação ser executada
    },
